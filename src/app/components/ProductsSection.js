...
export default function ProductsSection() {
  return (
    <section className="py-2 md:py-12 px-4 overflow-hidden" id='products'>
      <div className="max-w-7xl mx-auto">
        
       <div className="max-w-4xl mx-auto px-4 py-8 md:py-20 text-center">
  <div className="flex justify-center mb-2 md:mb-4">
    <span className="inline-block text-emerald-600 font-bold tracking-[0.25em] uppercase text-[8px] md:text-xs bg-emerald-50 px-4 py-1.5 rounded-full">
      Premium Ayurvedic Collection
    </span>
  </div>

  <h2 className="text-xl sm:text-4xl md:text-6xl lg:text-7xl font-black text-slate-900 leading-[1.1] tracking-tighter mb-2 md:mb-8">
  ...
  <p className="max-w-2xl mx-auto text-xs md:text-xl text-slate-500 leading-relaxed font-medium">
 ...
 <div className="grid gap-4 md:gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {products.map(product => (
            <Link
              ...
              className="group text-center md:text-left bg-slate-50 rounded-3xl p-2 md:p-6 border border-emerald-400 hover:border-emerald-200 transition-all"
            >
      <div className="relative w-full aspect-[7/8] md:aspect-[3/4] mb-3 md:mb-6 overflow-hidden rounded-2xl bg-white">
    <Image
      src={product.image}
      alt={product.name}
      fill
    ...
    />
              ...
              <p className="text-xs md:text-sm text-slate-500 mb-1 md:mb-4 italic">
                {product.tagline}
              </p>
              ...
              <div className="mt-2 md:mt-5 inline-block text-[10px] md:text-xs font-black uppercase tracking-widest text-emerald-600">
                View Details →
              </div>
            </Link>
          ))}
        </div>
<div className="md:mt-10 mt-5 md:pt-10 border-t border-slate-50">
